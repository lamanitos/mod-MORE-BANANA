![draw-4-banana-peels-there-will-be-an-angel-banana-in-](https://github.com/user-attachments/assets/e49b7ec7-cb6c-4c60-b443-6442220c9ca9)

# MORE BANANA!!!

* **The mod adds several bananas, namely:**
* **1.** Yellow Banana: a regular banana that can be slippery.
* **2.** Blue Banana: gives the Mandrill x2 speed (considering I added speed damage to the robot, this could be a problem). However, it has a debuff, making it easier for the robot to hit the yellow banana.
* **3.** Red Banana: gives the Mandrill a temporary berserk.
* **4.** Classified.

You can support me by simply visiting my Discord channel, where you'll find news on the development of other mods for other games, events, and news on the development of my own game.
